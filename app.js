...
const workerTutorialToggle = document.getElementById('workerTutorialToggle');
const workerTutorialBody   = document.getElementById('workerTutorialBody');
...
// Single source of truth for the Worker code shown in the tutorial — update
// it here and it updates everywhere it's displayed/copied.
//
// Two things this does that a plain passthrough proxy doesn't:
// 1. Sets a Referer + realistic browser headers server-side (something a
//    browser's own fetch() can't do cross-origin, but a Worker can).
// 2. Keeps a shared cookie jar (via the Cache API, built into Workers, no
//    extra setup) across requests. A known-working Python scraper for this
//    same site routes every request through one requests.Session(), which
//    automatically persists cookies — so once WeebCentral's Cloudflare sets
//    its trust cookie on the first request, every request after that looks
//    like the same returning visitor instead of a brand new anonymous one.
//    A stateless proxy re-rolls that "who is this?" check on every single
//    request, which is a big part of why it gets rate-limited so much
//    harder. This isn't a perfect global session (Cache API storage is
//    per-edge-location, not one single guaranteed store), but it's close
//    enough in practice to meaningfully help.
const WORKER_CODE = `export default {
  async fetch(request) {
    ...
    // --- Shared cookie jar (mimics requests.Session() cookie persistence) ---
    const cache = caches.default;
    const jarKey = new Request("https://cookie-jar.internal/weebcentral");
    let cookieHeader = "";
    const cached = await cache.match(jarKey);
    if (cached) cookieHeader = await cached.text();
    ...
    // If WeebCentral handed back a new/updated cookie, remember it for next time.
    const setCookie = upstream.headers.get("set-cookie");
    if (setCookie) {
      const newPair = setCookie.split(";")[0];
      const jar = new Map(
      ...
      const ni = newPair.indexOf("=");
      jar.set(newPair.slice(0, ni), newPair.slice(ni + 1));
      const merged = Array.from(jar, ([k, v]) => \`\${k}=\${v}\`).join("; ");
      await cache.put(jarKey, new Response(merged, { headers: { "Cache-Control": "max-age=1800" } }));
    }

    const out = new Headers();
    out.set("Access-Control-Allow-Origin", "*");
    out.set("Content-Type", upstream.headers.get("Content-Type") || "text/plain");
...
workerCodeEl.textContent = WORKER_CODE;
...
wireDisclosure(workerTutorialToggle, workerTutorialBody);
...
// ---------- copy buttons ----------
// Only handles buttons that declare data-copy or data-copy-target — copyLogBtn
// has its own dedicated handler above and deliberately isn't matched here.
// (Previously this fired for copyLogBtn too, immediately overwriting the
// clipboard with an empty string right after the real handler set it.)
document.addEventListener('click', (e) => {
  const btn = e.target.closest('.btn-copy');
  if(!btn) return;
  if(btn.dataset.copy === undefined && btn.dataset.copyTarget === undefined) return;
...
// Persistent adaptive pacing, modeled on a known-working Python scraper for
// this same site: rather than only reacting after a 429 with a one-off
// pause, keep a running "how fast are we allowed to go" delay.
//
// Important: this recovers gradually on a run of successes, not only after
// a full 60s of total silence. Requiring total silence basically never
// happens on a big series under sustained load, which is what caused the
// pacing to ratchet all the way up to its ceiling and get stuck there for
// the rest of the download in earlier testing.
let staggerMs = 200;
const STAGGER_MIN = 200;
const STAGGER_MAX = 2500;
let rateLimitHits = 0;
let consecutiveOk = 0;
const RECOVERY_STREAK = 12; // this many clean requests in a row -> ease off one notch

function registerRateLimit(){
  rateLimitHits++;
  consecutiveOk = 0;
  staggerMs = Math.min(STAGGER_MIN * Math.pow(1.3, rateLimitHits), STAGGER_MAX);
}
function registerSuccess(){
  if(rateLimitHits === 0) return;
  consecutiveOk++;
  if(consecutiveOk >= RECOVERY_STREAK){
    consecutiveOk = 0;
    rateLimitHits = Math.max(0, rateLimitHits - 1);
    staggerMs = rateLimitHits === 0 ? STAGGER_MIN : Math.min(STAGGER_MIN * Math.pow(1.3, rateLimitHits), STAGGER_MAX);
  }
}
...
// Accepts either a bare Worker URL ("https://x.workers.dev/") or an explicit
// template with a {url} placeholder ("https://x.workers.dev/?url={url}") —
// both should work, since beginners following the tutorial are told they
// can paste just the bare address. Gluing the target straight onto the end
// of a bare domain (the old behavior) produced a broken URL.
function buildCustomProxyUrl(template, target){
  const encoded = encodeURIComponent(target);
  if(template.includes('{url}')) return template.replace('{url}', encoded);
  if(/[?&](url=)/i.test(template)) return template + encoded; // already has "...url=" dangling
  const sep = template.includes('?') ? '&' : (template.endsWith('/') ? '?' : '/?');
...
function buildAttempts(){
  ...
  const customAttempt = custom ? {
    id: 'custom',
    label: 'your proxy',
    build: u => buildCustomProxyUrl(custom, u),
  } : null;
...
async function fetchResource(url, { binary = false, retries = 2, signal, extraHeaders, quiet = false } = {}){
  ...
  let lastErr;
  let rateLimitCountedThisCall = false;
    ...
    for(let tryNum = 0; tryNum <= attemptRetries; tryNum++){
      ...
      try{
        ...
        if(attempt.id === 'direct'){
          ...
          // Only count this once per logical request — otherwise 3 retries
          // of the same failing image each ratchet the pacing up on their
          // own, tripling the effect of what's really one rate-limit event.
          if(!rateLimitCountedThisCall){
            registerRateLimit();
            rateLimitCountedThisCall = true;
          }
          const cooldownMs = e.retryAfter ? parseFloat(e.retryAfter) * 1000 : 4000 + Math.random() * 2000;
          const until = Date.now() + cooldownMs;
          if(until > cooldownUntil){
            cooldownUntil = until;
            logEvent('warn', `Getting rate-limited (HTTP 429) — pausing ~${Math.round(cooldownMs / 1000)}s, pacing now 1 request every ${Math.round(staggerMs)}ms.`);
          }
        }else{
...
// ---------- concurrency pool ----------
// Shared launch gate: no request (across ALL concurrent lanes) fires within
// staggerMs of the previous one. This was previously computed but never
// actually enforced anywhere — the app only reacted after hitting a 429,
// never paced itself proactively, which is why it looked like bursts
// followed by pileups instead of a smooth, sustainable rate.
// Implemented as a chained mutex so concurrent lanes can't both read the
// gate before either updates it (a naive "check timestamp, then sleep"
// version would race under concurrency).
let lastLaunch = 0;
let paceChain = Promise.resolve();
function paceLaunch(){
  const turn = paceChain.then(async () => {
    const wait = (lastLaunch + staggerMs) - Date.now();
...
async function downloadAll(){
  ...
  try{
    ...
    const failedItems = []; // anything that failed the first pass, for a retry sweep later

    const chapterContexts = chapterImages.map(({ chapter, images }) => ({
      chapter, images,
      folder: zip ? zip.folder(sanitizeName(chapter.name)) : null,
      chapterBlobs: new Array(images.length),
    }));

    for(const ctx of chapterContexts){
      if(signal.aborted) break;

      await runPool(ctx.images, async (imgUrl, i) => {
        if(signal.aborted) return;
        try{
          const { data: blob } = await fetchResource(imgUrl, { binary: true, retries: 2, signal, quiet: true });
          ctx.chapterBlobs[i] = blob;
        }catch(e){
          if(e.name !== 'AbortError') failedItems.push({ ctx, i, imgUrl, chapterName: ctx.chapter.name });
        }
        done++;
        if(done % 25 === 0 || done === totalImages){
          logEvent('info', `Progress: ${done}/${totalImages} images downloaded`);
        }
        setProgress(Math.round((done / totalImages) * 85), `Downloading images… ${done}/${totalImages}`);
      }, 3, signal);
    }

    if(signal.aborted) throw new DOMException('Aborted', 'AbortError');

    // Nothing is silently dropped: anything that failed gets one more pass
    // once the rest of the download has let pacing settle back down.
    let stillFailed = [];
    if(failedItems.length > 0){
      logEvent('warn', `${failedItems.length} image(s) failed on the first pass — retrying now that things have settled.`);
      setProgress(87, `Retrying ${failedItems.length} image(s) that failed…`);
      await runPool(failedItems, async (item) => {
        if(signal.aborted) return;
        try{
          const { data: blob } = await fetchResource(item.imgUrl, { binary: true, retries: 3, signal, quiet: true });
          item.ctx.chapterBlobs[item.i] = blob;
        }catch(e){
          if(e.name !== 'AbortError') stillFailed.push(item);
        }
      }, 2, signal);

      if(stillFailed.length > 0){
        const byChapter = new Map();
        stillFailed.forEach(it => byChapter.set(it.chapterName, (byChapter.get(it.chapterName) || 0) + 1));
        logEvent('error', `${stillFailed.length} image(s) could not be downloaded and will be missing from the output:`);
        byChapter.forEach((count, name) => logEvent('error', `  · ${name} — ${count} missing`));
      }else{
        logEvent('ok', `All previously-failed images recovered on retry.`);
      }
    }

    const pdfBlobs = [];
    chapterContexts.forEach(ctx => {
      ctx.chapterBlobs.forEach((blob, i) => {
        if(!blob) return;
        if(ctx.folder) ctx.folder.file(`${String(i + 1).padStart(3, '0')}.${extFromResponse(ctx.images[i], blob)}`, blob);
    ...
    const missing = stillFailed.length;
    setProgress(100, missing > 0
      ? `Done — ${totalImages - missing}/${totalImages} images (${missing} missing, see log).`
      : `Done — ${totalImages} images across ${selected.length} chapter(s).`);
  }catch(e){
