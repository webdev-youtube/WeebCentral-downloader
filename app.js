...
// Sets a Referer + realistic browser headers server-side (something a
// browser's own fetch() can't do cross-origin, but a Worker can), which
// matches what a known-working reference scraper for this site does and
// helps avoid being flagged as a bot as quickly.
...
// pause, keep a running "how fast are we allowed to go" delay that grows
// each time we get rate-limited within the last minute, and decays back to
// baseline once things have been quiet. runPool reads this live, so the
// whole app slows down together and eases back off over time.
let staggerMs = 200;
let rateLimitHits = 0;
let lastRateLimitTime = 0;
function registerRateLimit(){
  const now = Date.now();
  rateLimitHits = (now - lastRateLimitTime < 60000) ? rateLimitHits + 1 : 0;
  lastRateLimitTime = now;
  staggerMs = Math.min(200 * Math.pow(1.5, rateLimitHits), 5000);
}
      ...
      try{
        ...
        if(attempt.id === 'direct'){
          ...
          if(until > cooldownUntil){
            cooldownUntil = until;
            logEvent('warn', `Getting rate-limited (HTTP 429) — pausing all requests for ~${Math.round(cooldownMs / 1000)}s, and slowing down to 1 request every ${staggerMs}ms for a while.`);
          }
        }else{
